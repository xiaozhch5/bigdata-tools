...
public class ShellUtil {
     ...
     * @return ShellResult
     */
    public static ShellResult sshUploadFile(String localFilePath, String remoteTargetDirectory, ClientHost clientHost) {
        ShellResult result = new ShellResult();
        ...
        try {
            ...
            conn.connect();
            // 用户登录验证
            boolean isAuthenticated = conn.authenticateWithPassword(clientHost.getUsername(), clientHost.getPassword());
            if (!isAuthenticated) {
                errorBuilder.append(String.format("Login failed: host:%s, username:%s, password:%s\n",
                        clientHost.getHost(), clientHost.getUsername(), clientHost.getPassword()));
            }
            else {
                // 检查本地文件是否存在
                ...
                if (file.exists()) {
                    ...
                    errorBuilder.append(String.format("The local file does not exist: %s\n", localFilePath));
                }
            }
        }
        catch (Exception e) {
                ...
                for (String cmd : cmds) {
                    ...
                    try {
                        ...
                        BufferedReader stdError = new BufferedReader(
                                new InputStreamReader(session.getStderr(), StandardCharsets.UTF_8));
